...
func _process(_delta):
	...
	if hasArrowOnInventory(): shootArrow()
	if checkArrowOnGround(): instanceArrowPoint()
	if get_tree().root.has_node("ArrowPoint"): arrowPointUpdate()
...
func hasArrowOnInventory():
	return (Input.is_action_just_released("Fire") and
	!get_tree().root.has_node("Arrow") and
	(GlobalVars.playerShootingTime > 10))
